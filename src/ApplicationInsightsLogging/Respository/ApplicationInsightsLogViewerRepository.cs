...
public class ApplicationInsightsLogViewerRepository : ILogViewerRepository
{
    ...
    public ApplicationInsightsLogViewerRepository(ILoggingConfiguration loggingConfiguration, ILogger<LogViewerRepository> logger, IJsonSerializer jsonSerializer, UmbracoFileConfiguration umbracoFileConfig)
    {
    ...
    public IEnumerable<ILogEntry> GetLogs(LogTimePeriod logTimePeriod, string? filterExpression = null)
    {
        ...
        return GetLogs(logTimePeriod).Result;
    }

    /// <inheritdoc />
    public LogLevelCounts GetLogCount(LogTimePeriod logTimePeriod)
    {
        var logs = GetLogs(logTimePeriod).Result; // TODO Group by?

        return new LogLevelCounts
        {
            Debug = logs.Count(x => x.Level == LogLevel.Debug),
            Error = logs.Count(x => x.Level == LogLevel.Error),
            Fatal = logs.Count(x => x.Level == LogLevel.Fatal),
            Information = logs.Count(x => x.Level == LogLevel.Information),
            Warning = logs.Count(x => x.Level == LogLevel.Warning)
        };
    }
    ...
    private async Task<IEnumerable<ILogEntry>> GetLogs(LogTimePeriod logTimePeriod/*, ILogFilter logFilter*/)
    {
        // TODO - reimplement logFilter

        string workspaceId = "";
        var client = new LogsQueryClient(new DefaultAzureCredential()); // Proper credentials
        var result = await client.QueryWorkspaceAsync(workspaceId, "AppTraces",
